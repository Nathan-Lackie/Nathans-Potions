...
CREATE TABLE
    public.catalog (
        ...
        CONSTRAINT catalog_unique_type UNIQUE (red, green, blue, dark),
        CONSTRAINT catalog_sku_name CHECK ((sku ~ '^[a-zA-Z0-9_]{1,20}$'::text)),
        CONSTRAINT catalog_valid_price CHECK (
            (
                (1 <= price)
                AND (price <= 500)
            )
        ),
        CONSTRAINT catalog_sum_100 CHECK (((((red + green) + blue) + dark) = 100))
    ) TABLESPACE pg_default;

ALTER TABLE public.catalog OWNER TO postgres;

INSERT INTO public.catalog
(sku,              name,                  red, green, blue, dark, price, desired_quantity, show_in_catalog) VALUES
('ORANGE_POTION', 'Fire Potion',          75,  25,    0,    0,    50,    NULL,             TRUE),
('GREEN_POTION',  'Druid''s Delight',     0,   100,   0,    0,    50,    NULL,             TRUE),
('DARK_POTION',   'Dark Potion',          0,   0,     0,    100,  75,    NULL,             TRUE),
('BLUE_POTION',   'Blue Potion',          0,   0,     100,  0,    50,    NULL,             TRUE),
('YELLOW_ORANGE', 'Yellow Orange Potion', 50,  50,    0,    0,    50,    NULL,             TRUE),
('RED_POTION',    'Fighter''s Fury',      100, 0,     0,    0,    50,    NULL,             TRUE);
...
CREATE TABLE
    public.potions (
        ...
        CONSTRAINT potions_sku_fkey FOREIGN KEY (sku) REFERENCES CATALOG (sku) ON UPDATE CASCADE ON DELETE CASCADE
    ) TABLESPACE pg_default;
...
CREATE TABLE
    public.carts (
        ...
        CONSTRAINT carts_id_fkey FOREIGN KEY (visit_id) REFERENCES visits (id),
        CONSTRAINT carts_potion_sku_fkey FOREIGN KEY (potion_sku) REFERENCES catalog (sku) ON UPDATE CASCADE
    ) TABLESPACE pg_default;
